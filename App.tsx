...
export default function App() {
    ...
    const generatePrompts = useCallback(() => {
        ...
        indo += `Detail kamera: ${environment.shotType}, ${environment.cameraAngle}.\n\n`;
        
        indo += `KARAKTER:\n`;
        characters.forEach((c, i) => {
            const race = c.race === 'Lainnya...' ? c.customRace : c.race;
            indo += `- Karakter ${i + 1}: Seorang ${c.gender} ras ${race} berusia ${c.age} tahun. Mengenakan ${c.outfit}. Gaya rambut ${c.hairstyle}. Aksi/Deskripsi: ${c.description}\n`;
        });
        ...
        characters.forEach((c, i) => {
            ...
            eng += `- Character ${i + 1}: A ${c.age}-year-old ${race} ${gender}. Wearing ${c.outfit}. ${c.hairstyle} hairstyle. Action/Description: ${c.description}\n`;
        });
        ...
        // Models often fail with raw data JSONs because they lack a clear "prompt" field.
        // We now construct a payload-ready JSON object.
        const jsonObject = {
            meta: {
            ...
            // The 'prompt' field is critical for compatibility with tools that expect a string input
            prompt: eng,
            negative_prompt: "bad quality, distorted, blurry, watermark, text overlay, bad anatomy, deformed, ugly, pixelated, low resolution",
            
            // Configuration parameters commonly used in video generation
            parameters: {
                aspect_ratio: "16:9",
                resolution: "1080p",
                frame_rate: 24,
            ...
            // Structured data for granular programmatic access if needed
            structured_data: {
                scene: {
    ...
    return (
        <div className="min-h-screen p-4 md:p-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Inputs Column */}
                <div>
                    {/* Section 1: Environment & Camera - Moved to top for better logical flow */}
